...
/// 昼夜 BGM（2026-08-12 双常驻循环源；2026-08-13 修订）：白天度改读
/// LightManager.CurrentSunlight01——太阳仰角正弦，即"此刻眼睛看到的日照"。
/// 不跟世界钟，也不跟墙钟：光照被手动调试拖时间时，音乐同样跟着眼前的天亮天黑走。
/// </summary>
public class AmbientAudio : MonoBehaviour
{
    ...
    [Header("昼夜 BGM（双源交叉淡化；白天度=场景实际日照 LightManager.CurrentSunlight01，只读）")]
    [Tooltip("背景音乐基准音量——比音效更低，宁可听不见不要盖过风声")]
    [Range(0f, 1f)] public float musicBaseVolume = 0.2f;
    [Tooltip("白天度平滑速率——日照跳变（调试拖时间/补跑回来）时音量也是爬过去，不瞬移")]
    ...
    private LightManager _lightManager;       // 日照来源（惰性查找，场景常驻组件）
    ...
    void Awake()
    {
        ...
        if (MusicDay       == null) MusicDay       = Resources.Load<AudioClip>("Audio/day");
        if (MusicDay       == null) MusicDay       = Resources.Load<AudioClip>("Audio/Days");
        if (MusicNight     == null) MusicNight     = Resources.Load<AudioClip>("Audio/music_night");
        if (MusicNight     == null) MusicNight     = Resources.Load<AudioClip>("Audio/night");
        if (MusicNight     == null) MusicNight     = Resources.Load<AudioClip>("Audio/Nights");
    ...
    void Update()
    {
        ...
        if (_lightManager == null) _lightManager = FindFirstObjectByType<LightManager>();
        if (_lightManager == null) return;

        // 白天度=场景实际日照（太阳仰角正弦）：日出日落是一段坡，不是一次换台；
        // 夜里有月光也不出声骗——仰角见负即是夜
        float target = _lightManager.CurrentSunlight01;
