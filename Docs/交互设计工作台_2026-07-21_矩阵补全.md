...
| 雨 → 蒲公英/石边草 | 蒲公英 ✅ 已激活（2026-07-23）；石边草仍惰性 | 【蒲公英侧已落地 2026-07-23】落种湿度门读 lowland.soilMoisture＝雨的间接下游（风大天干则絮飘了也不生），见 §5.3；石边草激活设计已入档，见 §5.3 | 低洼新绒苗（§5.3 痕迹链末端） |
...
| 蒲公英 → ? | ✅ 已落地 2026-07-23 | 【后续·2026-07-21 设计入档】**风的实体化**：WindSpeed 峰值日 → 种子絮飘散（粒子，玩家可见的"风"）→ 下风区（按拓扑）湿度合适时 N 日后出现新绒苗。生态位＝风向指示器 ✅ 实施（Batch 3）：L2 落种事件 DandelionSeedsDrifted（VegetationSystem.TickDandelion，风峰 0.7＋冷却 5 天＋湿度门 0.4，冷却从 append-only 日志回读）＋L3 絮粒子（Glimmer/SeedFluff shader，binder 推送 seedFluffRate）＋下风区新绒苗痕迹（4 日冒苗/15 日撤出，TraceKit.Sprout 程序化 mesh）＋场景九断言，play 实测全绿 2026-07-23；详见 Docs/更改_2026-07-22.md（追加二） | 飘絮本身＋下风处的新苗 |
| 石边草 → ? | 完全惰性 | 【后续·2026-07-21 设计入档】**旱的指示器**：响应最快——少量雨 24h 内转绿，长期无雨最先枯黄（stone_area 的活雨量计） | 颜色变化（复用草色通路，零新系统） |
| 候鸟在场 → ?（在场期间的正效应） | 仅狐狸 safety↓ | 【后续·2026-07-21 设计入档】候鸟在 riverbank → 虫害速率 −x（吃虫）。依赖 B3 | 语料为主＋高地枯黄斑块恢复可见 |
...
| 草色三合一（B2+B4+C4）【后续优先级 4】 | DecayLevel 无凋萎可视化；season 无视觉表达；GrassPreset SO 无读者 | 【2026-07-21 设计入档】GrassPreset 激活为唯一映射表：{DecayLevel, season, zone 湿度} → 草色/密度/健康——三个 B/C 级缺口一次闭合，不用做三遍 | 枯黄、季节色、旱黄共用一条视觉通路 |
| 树荫 → center 微气候【后续优先级 5】 | 无（断枝是一次性事件，无持续气候影响） | 【2026-07-21 设计入档】冠幅/vitality → center 水分消退减慢；断枝 → 遮荫↓ → 树下草更稀疏枯黄。依赖 C2/树视觉 | 树下草色与四周的对比 |
