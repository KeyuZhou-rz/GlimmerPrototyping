...
|---|---|---|---|
| 雨 → 田鼠 | 仅经 lowland 水位间接（庇护↓/搬家规则） | 雨造成土壤湿度 水位变化 mouse每tick读取相关值并作出反应 结构与其他焦虑值等相仿【切片已落地：`ActivityModifier(zone)`，§6-3】 | 雨毁巢穴 → **T1 土堆第四老化态「水毁」**（2026-07-21 定稿，【后续】实施）：土堆所在 zone soilMoisture＞0.8 时视觉切换为塌陷湿泥态（更矮、更暗、边缘糊），寿命照旧永久，不新增痕迹类型。与 T2 塌洞分工：水毁是痕迹的状态，塌洞是地貌的疤。三环：土堆怎么是塌的 → 前些天雨水把这片泡透了 → 田鼠搬去了高地 → 至今未回 |
...
| 旱（长期低雨）【后续优先级 2】 | 机制不存在（仅水位每日 −0.03 自然消退） | 【2026-07-21 设计入档】droughtDebt 累积：每 tick `+= max(0, 季节基准雨 − Rainfall)×k`，下雨扣减。三阈值：＞0.3 草黄、decay↑；＞0.6 地裂 decal＋动物向 riverbank 收敛＋树恢复停滞；雨季 debt 衰减。旱是"雨→生命"整张表的反向总开关 | 干裂地表、枯黄、**河岸脚印密度异常升高**（聚集本身就是旱的可读信号）、低洼水面退缩（B1 已有）。三环："河岸脚印怎么这么密？→ 旱了，都聚到水边 → 低洼的水退了一个月 → 上个雨季就没下透" |
