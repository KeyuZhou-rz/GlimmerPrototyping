...
| 植被 → 水保持【后续优先级 3】✅ 已落地 2026-07-23 | 无（目前矩阵里生命→环境无反向线） | 【2026-07-21 设计入档】vegetationDensity → soakRate ＋δ、日消退 −ε（两个系数，成本极低）。这是矩阵**第一条生命→环境的反向耦合** ✅ 实施（Batch 3）：drain＝max(0.03−0.015×veg, 0.005)、soak ＋0.05×veg（WorldManager 传播函数内，loc.waterLevel 单写者不变，同写者内部读 veg）；BehaviorNarrator 捂水语料 env_water_retention（雨停∧低洼水位>0.5∧veg>0.6，20 天冷却，暗示不点破）＋场景十断言；详见 Docs/更改_2026-07-23.md | 植被茂密区雨后水退更慢（水位视觉差异，B1 已有）＋语料/信解释 |
