...
| 雨 → 蒲公英/石边草 | 植物本身惰性 | 【切片不做】实体惰性，先不动；激活设计已入档（2026-07-21），见 §5.3 | — |
...
| 蒲公英 → ? | 完全惰性 | 【后续·2026-07-21 设计入档】**风的实体化**：WindSpeed 峰值日 → 种子絮飘散（粒子，玩家可见的"风"）→ 下风区（按拓扑）湿度合适时 N 日后出现新绒苗。生态位＝风向指示器 | 飘絮本身＋下风处的新苗 |
...
| 植被 → 水保持【后续优先级 3】 | 无（目前矩阵里生命→环境无反向线） | 【2026-07-21 设计入档】vegetationDensity → soakRate ＋δ、日消退 −ε（两个系数，成本极低）。这是矩阵**第一条生命→环境的反向耦合** | 植被茂密区雨后水退更慢（水位视觉差异，B1 已有）＋语料/信解释 |
